...
def macro(dataset, threshold, if_generate=False):
   ...
   for raw_dat in dataset:
       ...
       if if_generate:
           res_buffer = {'id': raw_dat['id'], 'premise': raw_dat['premise'], 'entity': raw_dat['entity'],
                         'annotation': raw_dat['annotation'], 'threshold': threshold, 'predicted_labels': list(predicted_labels)}
...
def main():
   ...
   for threshold in np.arange(args.threshold_start, 1.0+args.threshold_step, args.threshold_step):
       ...
       summary = f'Threshold = {round(threshold,3)}\t'\
                 ...
                 f'{round(round(f1(precision, recall), 3) * 100)}'
       print(summary)
   ...
   with open(os.path.join(args.model_dir,'result.json'), 'w+') as fout:
       fout.write("\n".join([json.dumps(items) for items in all_result]))
